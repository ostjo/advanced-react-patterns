...
The **Control Props** pattern gives users full control over the state of your
component. `<input />` components in React follow that pattern: They can either
be

- uncontrolled (no `value` and `onChange` prop is passed)
- controlled (both `value` and `onChange` prop is passed)

We can apply the same pattern to our custom component:
...
const MyToggle = () => {
  ...
  return <Toggle on={on} onChange={handleToggleChange} />
}
```
