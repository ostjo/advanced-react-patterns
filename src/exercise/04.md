...
A collection of props that cover common use-cases and prevents users from having
to wire everything up themselves.
...
const useToggle = () => {
  ...
  return {
    ...
    toggleProps: {
      'aria-pressed': on,
      onClick: toggle,
      // ... and more props that support accessibility / proper functionality
    },
  }
}
```
...
A function that returns a prop collection. This pattern actually allows for more
flexibility than prop collections.
...
const useToggle = () => {
  ...
  const getToggleProps = ({onClick, ...props} = {}) => ({
    'aria-pressed': on,
    onClick: () => {
      // custom onClick function that users can optionally provide
      onClick?.();
      // our standard toggle function
      toggle();
    }
    ...props,
  })
