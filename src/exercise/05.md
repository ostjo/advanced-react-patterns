...
The **State Reducer Pattern** is a pattern of handing over state control of your
component to the delevoper using it (_"Inversion of Control"_). It enables
people to modify how the component updates its state internally.

We provide a default reducer (e.g. `toggleReducer`) that the developer can
extend to their needs, like so:

```js
function customToggleStateReducer(state, action) {
  // here's our custom action & state modification triggered by an event
...
// here we pass our custom reducer to the useToggle hook
const {on, getTogglerProps, getResetterProps} = useToggle({
  reducer: customToggleStateReducer,
})
```

> Note that in the above case we provide a `toggleActionTypes` object to prevent
> users making spelling errors and providing unsupported action types.
