...
Users of reducers (`Counter`) often define their own functions to update state
(`increment()`), like so:
...
It's not the best API and especially becomes an annoyance when you have a
**sequence of dispatches** to call. We could create helper functions instead and
...
return <CounterContext.Provider value={{state, increment}} {...props} />
```

But, as Dan Abramov says:
...
He suggests to create an importable "helper" that accepts `dispatch`. In the
end, that could look smth like this:
...
- can
  [improve performance](https://twitter.com/dan_abramov/status/1125774170154065920)
- helps you avoid mistakes in dependency lists

---

> 💡 Tip: You can give contexts names that will be displayed in React DevTools
> (per default they are all displayed as "Context.Provider")
>
> ```js
