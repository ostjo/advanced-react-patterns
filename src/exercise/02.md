...
Compound components are a **set of components** that in **combination** form a
complete UI (e.g. like `<select>` and `<option>` in HTML).

The **Compound Components Pattern** provides a set of components in a simple yet
powerful, declarative way. Like so:

```javascript
function App() {
  return (
    <Toggle>
      <ToggleOn>I am visible when "on"<ToggleOn />
      <ToggleOff>I am visible when "off"<ToggleOff />
    <Toggle />
  )
}
```

The `<Toggle />` will implicitly provide the necessary props to its children by
mapping them:
...
In case we also want to support DOM components as `children` (that do not accept
props like `on` or `toggle`), we can check for DOM vs composite elements like
so:

```js
// DOM components are of type string, while composite components are functions
...
> **💡 Tip:**
>
> - DOM component: `<span>`, `<div>`, `<option>`, ...
> - composite component: `<Toggle />`, `<App />`, `<Button />`

> **💡 Tip:**
>
> We can restrict the child components that we pass our internal props to by
...
> ```
>
> ... and then cloning children conditionally:
>
> ```js
> ...
